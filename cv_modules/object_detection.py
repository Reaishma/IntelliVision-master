...
def detect_objects_opencv(image, confidence_threshold=0.5):
    ...
    try:
        ...
        # Calculate dynamic area threshold based on image size (0.1% of total area)
        min_area = (width * height) * 0.001
        
        for i, contour in enumerate(contours[:20]):  # Check more contours
            area = cv2.contourArea(contour)
            if area > min_area:  # Filter very small areas
                x, y, w, h = cv2.boundingRect(contour)
                # Calculate confidence based on size and aspect ratio
                area_ratio = area / (width * height)
                aspect_ratio = w / h if h > 0 else 1
                
                # Higher confidence for reasonably sized objects with good aspect ratios
                base_confidence = 0.6
                area_score = min(0.25, area_ratio * 50)  # Up to 0.25 based on area
                aspect_score = 0.1 if 0.2 < aspect_ratio < 5 else 0  # Bonus for reasonable shapes
                
                confidence = min(0.95, base_confidence + area_score + aspect_score)
