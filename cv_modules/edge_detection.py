...
def log_edge_detection(image, sigma=1.0):
    ...
    # Apply Laplacian (output is float64)
    laplacian = cv2.Laplacian(blurred, cv2.CV_64F)
    
    # Normalize and convert to uint8 for edge detection
    # Take absolute value and normalize to 0-255 range
    abs_laplacian = np.absolute(laplacian)
    edges = np.uint8(np.clip(abs_laplacian * 255 / abs_laplacian.max() if abs_laplacian.max() > 0 else abs_laplacian, 0, 255))
    ...
    if st.button("🚀 Detect Edges", type="primary"):
        with st.spinner(f"Applying {method} edge detection..."):
            ...
            try:
                ...
                with col2:
                    st.subheader("Edges")
                    st.image(edges, caption=f"{method} Edges", use_container_width=True, clamp=True)
                ...
                with col3:
                    ...
                    st.image(overlay, caption="Edge Overlay", use_container_width=True)
                ...
                if contours:
                    ...
                    if significant_contours:
                        ...
                        st.image(img_with_contours, caption="Detected Contours", use_container_width=True)
